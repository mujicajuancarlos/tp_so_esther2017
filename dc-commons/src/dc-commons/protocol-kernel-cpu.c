...
char* serialize_stack(t_stack_program** contextos, uint32_t contextos_length) {
	...
	t_stack_program* aux_contextos = *contextos;
		...
		for (i = 0; i < contextos_length; i++) {
			...
			serialize_and_copy_value(stream,&(size_contexto),sizeof(uint32_t),&offset);//size contexto
			serialize_and_copy_value(stream,serialized_contexto,sizeof(char)*size_contexto,&offset);//contexto
...
t_stack_program* deserialize_stack(char* stream, uint32_t contextos_length) {
	...
	int offset = 0;
		...
		for (i = 0; i < contextos_length; i++) {
			...
			deserialize_and_copy_value(&(size_contexto),stream,sizeof(uint32_t),&offset);
			char* serialized_contexto = malloc(sizeof(char)*size_contexto);
			//contextos = realloc(contextos,sizeof(contexto)*(i+1));
			deserialize_and_copy_value(serialized_contexto,stream,size_contexto,&offset);
...
void crearNuevoContexto(PCB* pcb){
	pcb->stack = realloc(pcb->stack,sizeof(t_stack_program)*(pcb->stackSize+1));
...
void destruirContextoActual(PCB* pcb, int size_pagina){
	...
	pcb->stack = realloc(pcb->stack,sizeof(t_stack_program)*(pcb->stackSize));
}
...
char* serializar_contexto(t_stack_program* contexto){
	...
	serialize_and_copy_value(stream,serialized_dictionary,sizeof(t_variable)*contexto->arg_len,&offset);
	free(serialized_dictionary);

	//cantidad de variables
	serialize_and_copy_value(stream,&(contexto->var_len),sizeof(uint32_t),&offset);

	//serializar array de variables
	serialized_dictionary = serializar_array_variables(&(contexto->variables),contexto->var_len);
	serialize_and_copy_value(stream,serialized_dictionary,sizeof(t_variable)*contexto->var_len,&offset);
	free(serialized_dictionary);

	//posicion de retorno
	serialize_and_copy_value(stream,&(contexto->retPos),sizeof(t_puntero_instruccion),&offset);

	//direccion de variable de retorno
	serialize_and_copy_value(stream,&(contexto->retVar.pagina),sizeof(uint32_t),&offset);
	serialize_and_copy_value(stream,&(contexto->retVar.offset),sizeof(uint32_t),&offset);
	serialize_and_copy_value(stream,&(contexto->retVar.size),sizeof(uint32_t),&offset);
...
t_stack_program* deserializar_contexto(char* stream){
	...
	char* serialized_dictionary = malloc((sizeof(t_variable))*context->arg_len);
	deserialize_and_copy_value(serialized_dictionary,stream,(sizeof(t_variable))*context->arg_len,&offset);

	context->argumentos = deserializar_array_variables(serialized_dictionary,context->arg_len);
	free(serialized_dictionary);

	//cantidad de variables
	deserialize_and_copy_value(&(context->var_len),stream,sizeof(uint32_t),&offset);

	serialized_dictionary = malloc((sizeof(t_variable))*context->var_len);
	deserialize_and_copy_value(serialized_dictionary,stream,(sizeof(t_variable))*context->var_len,&offset);

	context->variables = deserializar_array_variables(serialized_dictionary,context->var_len);
	free(serialized_dictionary);

	deserialize_and_copy_value(&(context->retPos),stream,sizeof(t_puntero_instruccion),&offset);

	deserialize_and_copy_value(&(context->retVar.pagina),stream,sizeof(uint32_t),&offset);
	deserialize_and_copy_value(&(context->retVar.offset),stream,sizeof(uint32_t),&offset);
	deserialize_and_copy_value(&(context->retVar.size),stream,sizeof(uint32_t),&offset);
...
char* serializar_array_variables(t_variable** variables, uint32_t len){
	...
	int i;
	for (i = 0; i < len; i++) {
		serialize_and_copy_value(stream,&(aux_variables[i].nombre),sizeof(char),&offset);
		serialize_and_copy_value(stream,&(aux_variables[i].direccion.pagina),sizeof(uint32_t),&offset);
		serialize_and_copy_value(stream,&(aux_variables[i].direccion.offset),sizeof(uint32_t),&offset);
		serialize_and_copy_value(stream,&(aux_variables[i].direccion.size),sizeof(uint32_t),&offset);
	}
	return stream;
}
...
t_variable* deserializar_array_variables(char* stream, uint32_t len){
	...
	for(i=0; i<len; i++){
		...
		deserialize_and_copy_value(&(variables[i].nombre),stream,sizeof(char),&offset);
		deserialize_and_copy_value(&(variables[i].direccion.pagina),stream,sizeof(uint32_t),&offset);
		deserialize_and_copy_value(&(variables[i].direccion.offset),stream,sizeof(uint32_t),&offset);
		deserialize_and_copy_value(&(variables[i].direccion.size),stream,sizeof(uint32_t),&offset);
	}
	return variables;
}
/******************************************************************************************************************
